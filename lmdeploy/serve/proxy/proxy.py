...
from lmdeploy.pytorch.disagg.messages import PDConnectionMessage, EPConnectionMessage
...
class NodeManager:
    ...
    def __init__(self,
        ...
        # bugfix: ep connection pool should use EPConnectionPool not PDConnectionPool
    ...
    if node_manager.serving_strategy == ServingStrategy.Hybrid:
        ...
        if len(node_manager.encoder_nodes):
            if _need_encoder(request_dict.get('messages', [])):
                ...
                if not encoder_url:
                    logger.warning('Encoder nodes registered but no suitable encoder node found for model; skip encoder stage.')
                else:
                    ...
                    encoder_response_text = await node_manager.generate(request_dict, encoder_url, '/v1/chat/completion')
